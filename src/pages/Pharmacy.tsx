...
import { Patient, Prescription, Drug, UserRole } from '../types';
...
import { collection, updateDoc, doc, serverTimestamp, query, orderBy, onSnapshot, addDoc, deleteDoc, getDocs } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { seedInventory } from '../lib/seedData';

// Role guard
const REQUIRED_ROLE: UserRole = 'Pharmacy';

export const Pharmacy: React.FC<{ userRole?: UserRole }> = ({ userRole }) => {
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[400px] space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-500 max-w-xs">Only Pharmacy staff can access this page.</p>
          </div>
  ...
  // Subscribe to patients waiting for pharmacy (status = 'at_pharmacy')
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsubInventory = onSnapshot(q, (snapshot) => {
      ...
      setDrugs(dbDrugs);
      // Auto-seed if empty
      if (snapshot.empty) seedInventory();
    });
  ...
  useEffect(() => {
    ...
    const unsubPresc = onSnapshot(q, (snapshot) => {
      ...
      const pending = prescriptions.find(p => p.status === 'pending') || null;
      setPendingPrescription(pending);
    });
  ...
  const handleDispense = async () => {
    if (!selectedPatientData || !pendingPrescription) {
      setError('No pending prescription found.');
      return;
    }

    const pharmacyFee = 2500;
    if (selectedPatientData.walletBalance < pharmacyFee) {
      setError(`Insufficient wallet balance (₦${selectedPatientData.walletBalance?.toLocaleString()}) to cover ₦${pharmacyFee} pharmacy fee.`);
      return;
    }
    ...
    try {
      // 1. Check if all medications exist in inventory and have stock
      ...
      for (const med of pendingPrescription.medications) {
        ...
        } else if (matchingDrug.stock <= 0) {
          missingMeds.push(`${med} (out of stock)`);
        }
      }
      if (missingMeds.length > 0) {
        throw new Error(`Cannot dispense: ${missingMeds.join(', ')}`);
      }

      // 2. Mark prescription as dispensed
      await updateDoc(doc(db, `patients/${selectedPatientData.id}/prescriptions`, pendingPrescription.id!), {
        status: 'dispensed',
        updatedAt: serverTimestamp()
      });

      // 3. Decrement stock from inventory for each medication
      for (const med of pendingPrescription.medications) {
        const matchingDrug = drugs.find(d => d.name.toLowerCase().trim() === med.toLowerCase().trim());
        if (matchingDrug) {
          const newStock = Math.max(0, matchingDrug.stock - 1);
      ...
      // 4. Deduct pharmacy fee and move to completed
      ...
      // 5. Audit log
      await PatientService.logAction('Pharmacy', 'Pharmacy', `Dispensed prescription: ${pendingPrescription.medications.join(', ')}`, selectedPatientData.id);
    ...
    <Layout>
      <div className="space-y-8 max-w-7xl mx-auto">
        ...
        {activeTab === 'dispensing' ? (
              ...
              <AnimatePresence mode="wait">
                {currentPatient ? (
                        ...
                        <div className="p-8 space-y-8 bg-white overflow-y-auto max-h-[500px] scrollbar-hide">
                                  ...
                                  <div key={i} className="bg-white px-4 py-3 rounded-xl border border-slate-200 flex items-center justify-between shadow-sm group hover:border-emerald-200 transition-colors">
                                    ...
                                    {drug ? (
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Stock: {drug.stock}</span>
                                      </div>
                                    ) : (
                                      <span className="text-[9px] font-black text-amber-500 uppercase tracking-widest">Not in inventory</span>
                                    )}
                                  </div>
                                );
                              })}
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="text-right">
                                  ...
                                  <div className={`text-xl font-black tracking-tighter ${(currentPatient.walletBalance || 0) >= 2500 ? 'text-emerald-600' : 'text-red-500'}`}>
                                    ₦{(currentPatient.walletBalance || 0).toLocaleString()}
                                  </div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col justify-center gap-4">
                              ...
                              <button 
                                disabled={loading || (currentPatient.walletBalance || 0) < 2500}
                                ...
                                className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 shadow-xl transition-all ${
                                  (currentPatient.walletBalance || 0) >= 2500 && !loading
                              ...
                              >
                                ...
                                {loading ? 'Processing...' : 'Confirm Dispense & Debit'}
                              </button>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="h-full min-h-[500px] border border-slate-200 rounded-3xl flex flex-col items-center justify-center p-20 text-center bg-slate-50/30 shadow-inner">
                    ...
                    <p className="text-slate-500 max-w-sm mt-2 text-sm font-medium leading-relaxed">Authorized e‑prescriptions are queued here for fulfillment. Dispensation triggers automatic wallet transaction and inventory update.</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
          ...
          /* Inventory Matrix View – unchanged but with error display */
