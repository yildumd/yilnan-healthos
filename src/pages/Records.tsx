...
// Role guard – only Records should access this page
const REQUIRED_ROLE: UserRole = "Records";

export const Records: React.FC<{ userRole?: UserRole }> = ({ userRole }) => {
        ...
        <div className="flex flex-col items-center justify-center min-h-[400px] space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-500 max-w-xs">
              This page is only accessible to Records officers.
            </p>
          </div>
          <button
            onClick={() => navigate("/")}
            className="px-6 py-2 bg-slate-900 text-white rounded-md text-[10px] font-black uppercase tracking-widest"
          >
            Return to Dashboard
          </button>
        </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await PatientService.registerPatient(patientData, "RECORDS_STAFF");
  ...
  return (
    <Layout>
      <div className="space-y-6 md:space-y-8 max-w-7xl mx-auto">
                            ...
                            <td className="p-4 text-right">
                              <span
                                className={`text-xs font-black tracking-tight ${
                                  isLowBalance ? "text-red-600" : "text-slate-900"
                                }`}
                              >
                                ₦{p.walletBalance?.toLocaleString() || 0}
                              </span>
                            </td>
            ...
            <div className="bg-slate-900 rounded-xl p-6 text-white overflow-hidden relative group shadow-xl">
              <div className="relative z-10">
                ...
                <div className="text-4xl font-black tracking-tighter">
                  {allPatients.length}+{" "}
                  <span className="text-xs text-slate-500 font-bold uppercase">Files</span>
                </div>
              </div>
        ...
        {/* Registration Modal - Improved with more fields */}
        <AnimatePresence>
          {showRegForm && (
              ...
              >
                <div className="bg-slate-900 p-6 md:p-8 text-white relative shrink-0">
                  ...
                  <p className="text-slate-400 text-xs md:text-sm mt-1 font-medium">
                    Verify credentials and capture biometric identity for file creation.
                  </p>
                  <button
                ...
                <div className="p-6 border-t border-slate-100 bg-slate-50 flex items-center gap-3">
                  ...
                  <button
                    ...
                    disabled={submitting}
                    type="submit"
                    className="flex-[2] h-12 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {submitting ? <Loader2 size={16} className="animate-spin" /> : <UserPlus size={16} />}
                    {submitting ? "Creating..." : "Authorize File Creation"}
                  </button>
                </div>
              </motion.div>
            </div>
