...
// Optional role guard
const REQUIRED_ROLE: UserRole = 'Doctor';

export const Doctor: React.FC<{ userRole?: UserRole }> = ({ userRole }) => {
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[400px] space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-500 max-w-xs">Only Doctors can access this page.</p>
          </div>
  ...
  // Subscribe to patients waiting for doctor
  useEffect(() => {
    const unsubQueue = PatientService.subscribeToQueue('waiting_doctor', setPatients);
    return () => unsubQueue();
  }, []);

  // Real‑time subscription to selected patient's data (status, balance, etc.)
  useEffect(() => {
    if (!selectedPatient) {
    ...
    const unsubPatient = onSnapshot(patientRef, async (snap) => {
      if (snap.exists()) {
        const data = { id: snap.id, ...snap.data() } as Patient;
        setSelectedPatientData(data);
        // Fetch latest vitals
        const vitalsData = await PatientService.getSubcollection<Vitals>(data.id, 'vitals');
  ...
  const handleSaveNotesOnly = async () => {
    ...
    if (!consultation.diagnosis) {
      setError("Please enter a diagnosis before saving.");
      return;
    }
    setLoading(true);
    setError('');
    try {
      await PatientService.addConsultation(selectedPatientData.id, {
        diagnosis: consultation.diagnosis,
        treatmentPlan: consultation.treatmentPlan,
        clinicalNotes: consultation.clinicalNotes,
        doctorId: 'DR_YILNAN', // In real app, get from auth
      });
      // Optionally save directives in a subcollection or as part of consultation
      setError('');
      alert("Consultation saved (not yet submitted).");
    } catch (err: any) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleSubmitConsultation = async () => {
    if (!selectedPatientData) return;
    if (!consultation.diagnosis) {
      setError("Please enter a diagnosis before submitting.");
      return;
    }
    ...
    try {
      ...
      if (labRequest) {
        await PatientService.addLabRequest(selectedPatientData.id, {
          testType: labRequest,
          doctorId: 'DR_YILNAN',
      ...
      if (prescription.length > 0) {
        await PatientService.addPrescription(selectedPatientData.id, {
      ...
      // 4. Deduct consultation fee (e.g., ₦5000)
      const consultationFee = 5000;
      if ((selectedPatientData.walletBalance || 0) >= consultationFee) {
        await PatientService.debitAccount(
          selectedPatientData.id,
          consultationFee,
          'Consultation Fee',
          'DOCTOR_STAFF',
          undefined // Don't change status here; we'll update after all actions
        );
      } else {
        // Optionally, still allow but note insufficient funds – depends on policy
        setError(`Insufficient wallet balance (₦${selectedPatientData.walletBalance?.toLocaleString()}) to cover ₦${consultationFee} consultation fee. Please fund wallet.`);
        setLoading(false);
        return;
      }

      // 5. Determine next status
      let nextStatus: 'waiting_doctor' | 'sent_lab' | 'at_pharmacy' | 'completed' = 'completed';
      if (labRequest) {
        nextStatus = 'sent_lab';
      ...
      await PatientService.updateStatus(selectedPatientData.id, nextStatus, 'Doctor');

      // 7. Log audit (already inside addConsultation, etc., but add an explicit one)
      await PatientService.logAction('Doctor', 'Doctor', `Consultation completed for ${selectedPatientData.fullName}`, selectedPatientData.id);
            ...
            <AnimatePresence mode="wait">
              {currentPatient ? (
                ...
                >
                  ...
                  {/* Vitals Summary Card */}
                  {latestVitals && (
                    <div className="bg-purple-50/30 border border-purple-100 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <Activity size={20} className="text-purple-600" />
                        <span className="text-[10px] font-black text-purple-900 uppercase tracking-widest">Latest Vitals</span>
                      </div>
                      <div className="flex gap-6 text-xs">
                        <div><span className="font-bold text-slate-500">BP:</span> {latestVitals.bp}</div>
                        <div><span className="font-bold text-slate-500">Temp:</span> {latestVitals.temp}°C</div>
                        <div><span className="font-bold text-slate-500">Pulse:</span> {latestVitals.pulse} bpm</div>
                        <div><span className="font-bold text-slate-500">Weight:</span> {latestVitals.weight} kg</div>
                      </div>
                      <div className="text-[9px] text-slate-400">Recorded: {latestVitals.timestamp?.toDate?.().toLocaleDateString() || 'N/A'}</div>
                    </div>
                  )}
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden">
                    ...
                    <div className="p-8 bg-white overflow-y-auto max-h-[500px] scrollbar-hide">
                      {error && (
                        <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-2 text-red-700 text-xs">
                          <AlertCircle size={14} />
                          {error}
                        </div>
                      )}
