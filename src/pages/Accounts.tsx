...
export const Accounts: React.FC = () => {
  ...
  const handleTransaction = async () => {
    ...
    try {
      if (actionType === 'fund') {
        await PatientService.fundWallet(selectedPatient.id, val, 'ACCOUNTS_STAFF');
      } else {
        // Debiting consultation fee as requested in Step 2
        await PatientService.debitAccount(selectedPatient.id, val, 'Consultation Fee', 'ACCOUNTS_STAFF', 'ready_nursing');
                  ...
                  patients.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${p.walletBalance < 2000 ? 'bg-orange-50 border-orange-100 text-orange-500' : 'bg-slate-50 border-slate-100 text-slate-400 group-hover:bg-blue-50 group-hover:border-blue-100 group-hover:text-blue-500'}`}>
                          {p.walletBalance < 2000 ? <AlertCircle size={20} /> : <Wallet size={20} />}
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-8">
                        <div className="text-right">
                          <div className="text-sm font-black text-slate-900 tracking-tighter">₦{p.walletBalance.toLocaleString()}</div>
              ...
              {selectedPatient ? (
                ...
                >
                  <div className="p-6 bg-slate-900 text-white relative">
                    ...
                    <h3 className="text-xl font-black tracking-tight mb-6">{selectedPatient.fullName}</h3>
                    ...
                    <div className="bg-white/5 border border-white/5 rounded-xl p-5 flex items-center justify-between shadow-inner">
                      <div>
                        ...
                        <p className="text-2xl font-black tracking-tighter">₦{selectedPatient.walletBalance.toLocaleString()}</p>
                      </div>
                    ...
                    {actionType && (
                      ...
                      >
                         ...
                         <button 
                          ...
                          className={`w-full py-4 rounded-xl font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 shadow-xl transition-all ${actionType === 'fund' ? 'bg-emerald-600 text-white shadow-emerald-100 hover:bg-emerald-700 active:scale-95' : 'bg-red-600 text-white shadow-red-100 hover:bg-red-700 active:scale-95'}`}
                         >
                            {actionType === 'fund' ? <Plus size={16} /> : <Receipt size={16} />}
                            Authorize {actionType === 'fund' ? 'Deposit' : 'Debit'}
                         </button>
                      </motion.div>
                    )}
                            ...
                            <div key={tx.id} className="flex items-center justify-between p-3 rounded-xl bg-white border border-slate-100 shadow-sm">
                               <div className="flex items-center gap-3">
                                 ...
                                 <div className="flex flex-col">
                                   ...
                                   <span className="text-[8px] text-slate-400 font-bold uppercase tracking-widest">{new Date(tx.timestamp?.seconds * 1000).toLocaleDateString()}</span>
                                 </div>
                               </div>
