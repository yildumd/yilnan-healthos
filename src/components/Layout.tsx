...
// Define navigation items with role-based visibility and path
interface NavItem {
  id: string;
  label: string;
...
const navItems: NavItem[] = [
  { id: 'records', label: 'Records', icon: Users, path: '/records', allowedRoles: ['Records', 'Admin'] },
  { id: 'accounts', label: 'Accounts', icon: CreditCard, path: '/accounts', allowedRoles: ['Accounts', 'Admin'] },
  { id: 'nursing', label: 'Nursing', icon: HeartPulse, path: '/nursing', allowedRoles: ['Nurse', 'Admin'] },
  { id: 'doctor', label: 'Consultation', icon: Stethoscope, path: '/doctor', allowedRoles: ['Doctor', 'Admin'] },
  { id: 'lab', label: 'Laboratory', icon: Beaker, path: '/lab', allowedRoles: ['Lab Technician', 'Admin'] },
  { id: 'pharmacy', label: 'Pharmacy', icon: Pill, path: '/pharmacy', allowedRoles: ['Pharmacy', 'Admin'] },
  { id: 'admin', label: 'Administration', icon: Settings, path: '/admin', allowedRoles: ['Admin'] },
  // Generic patient file search – accessible to all clinical staff
  { id: 'patient-files', label: 'Patient Files', icon: FolderOpen, path: '/search', allowedRoles: ['Records', 'Nurse', 'Doctor', 'Lab Technician', 'Pharmacy', 'Accounts', 'Admin'] },
];
    ...
    <div className="flex flex-col h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden selection:bg-blue-100">
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 shrink-0 z-50">
        ...
        <div className="flex items-center gap-4 md:gap-8">
          <form onSubmit={handleGlobalSearch} className="relative group hidden xl:block">
            ...
            <nav className="space-y-1.5">
              {visibleNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon size={16} className={active ? 'text-blue-400' : 'text-slate-300 group-hover:text-slate-900'} />
                    {item.label}
                  </button>
                );
              })}
